...
| 12 | `10-tests.sh` | Run 75 verification tests, save results to `~/logs/boot-test-results.txt` | Yes — read-only tests | ~30s |
...
| `~/logs/boot-test-results.txt` | Full test results (75 PASS/FAIL/WARN checks) |
...
7. **Test on every boot** — `10-tests.sh` runs 75 checks and saves results for the PO to review.
