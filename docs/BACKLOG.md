...
**Maintained by:** TPM
**Last updated:** 2026-06-02 (F-0131 added and completed — install VS Code + reconcile home.nix drift)
...
| ID | Feature | Spec | Priority | Status | Owner | Branch | Dependencies | Feedback |
|----|---------|------|----------|--------|-------|--------|--------------|----------|
| F-0131 | Install VS Code via Nix + reconcile home.nix drift (fold bulk of F-0127) | [F-0131](specs/F-0131-install-vscode.md) | P1 | done | SWE-1 | feature/install-vscode | F-0123, F-0126, F-0127 | **Done (2026-06-02).** Installed `pkgs.vscode` via live home.nix. Reconciled minimal 30-line home.nix to full-profile: BASE_PKGS + vscode (no other IDEs), programs.zsh (aliases, initContent, zsh plugins), home.sessionVariables (EDITOR/VISUAL/BROWSER), programs.starship. Adopted sway config home.file directive (source existed + byte-matched live; backed up pre-switch). Adopted nvim config home.file directive (deployed canonical `workstation-image/configs/nvim/init.lua` as source; nvim installed but had no config). Omitted waybar directives (box uses swaybar; source files absent). `home-manager switch` succeeded cleanly. `code --version` on PATH. Sway reload confirmed clean. 4 other IDEs NOT installed. Added VS Code version check to 10-tests.sh. Boot tests: VSCode binary + version + sway-grep checks all PASS. Residual: stale `idea-oss/cursor/windsurf/zeditor` test FAILs remain → F-0126. IDE_PKGS in cloud-build-setup.sh still has 5 IDEs for fresh setup → F-0126. |
