...
| ID | Feature | Spec | Priority | Status | Owner | Branch | Dependencies | Feedback |
|----|---------|------|----------|--------|-------|--------|--------------|----------|
| F-0131 | Install VS Code via Nix + reconcile home.nix drift + autostart on workspace 2 | [F-0131](specs/F-0131-install-vscode.md) | P1 | done | SWE-1 | feature/install-vscode | F-0123, F-0126, F-0127 | **Done (2026-06-02).** Installed `pkgs.vscode` via live home.nix. Reconciled minimal 30-line home.nix to full-profile: BASE_PKGS + vscode (no other IDEs), programs.zsh (aliases, initContent, zsh plugins), home.sessionVariables (EDITOR/VISUAL/BROWSER), programs.starship. Adopted sway config home.file directive. Adopted nvim config home.file directive. Omitted waybar directives (box uses swaybar). `home-manager switch` succeeded cleanly. `code --version` on PATH. **Extended (2026-06-02):** Added sway-native VS Code autostart — `for_window [app_id="code"] move container to workspace number 2` placement rule + `exec env -u LD_LIBRARY_PATH $nix/code ...` autostart in sway AUTOSTART section. `ws-autolaunch.service` deliberately NOT used (masked by `11-custom-tools.sh`; that masking is intentional end-state of F-0106→F-0124 saga). `home-manager switch` + `swaymsg reload` confirmed clean. Runtime validation: VS Code landed on ws2 (confirmed via `swaymsg -t get_tree`). Three-places parity verified (repo + home-manager source byte-identical). New 10-tests.sh assertions PASS. Cold-boot confirmation deferred to next reboot. Residual: stale `idea-oss/cursor/windsurf/zeditor` test FAILs remain → F-0126. |
...
| F-0127 | Reconcile home.nix pattern checks vs actual config locations (9 FAILs) | — | P2 | partial | SWE-1 | — | F-0074, F-0131 | Surfaced 2026-06-02 during F-0123 cold-boot validation. **Partially resolved by F-0131 (2026-06-02):** F-0131 added programs.zsh (with zshrc.local sourcing, Nix profile, Timezone, Go GOROOT, Rust cargo/bin, pyenv init, rbenv init, starship prompt, tmux aliases) and home.sessionVariables to the live home.nix — these patterns should now be present in home.nix and the related boot-test pattern checks should flip to PASS after next boot/home-manager switch. **Remaining scope for F-0127:** verify the 9 pattern checks pass after F-0131's home-manager switch; update any checks that still fail because they grep the wrong file path; handle waybar vs swaybar directive divergence between generated and live home.nix. |
...
| F-0130 | Review and resolve boot test WARNs: GH Copilot extension + gh auth (2 WARNs) | — | P2 | superseded | SWE-1 | — | F-0067 | **Superseded by F-0138.** GitHub Copilot CLI removed entirely from the workstation setup and boot script validation, resolving the warning. |
...
## Milestone 47: Remove GitHub Copilot CLI
...
| F-0123 | 07-apps user-session wait timeout (120s) too short — app updates skipped on slow boots | [F-0123](specs/F-0123-app-updates-systemd-ordering.md) | P1 | done | SWE-1 | feature/f0123-dbus-probe-uid | F-0121 | **Done (2026-06-02, follow-up fix 2026-06-02).** Original PR #36: created `ws-app-updates.service` ordered `After=user@1000.service`. **Follow-up fix (feature/f0123-dbus-probe-uid):** original merge still SKIPped on every boot — 3 bugs remained: (1) D-Bus probe ran as root (uid 0) and session bus rejected it — fixed by wrapping probe in `runuser -u "$USER" --`; (2) boot-test race gave false PASS by sampling log mid-run — fixed with `systemctl show -p SubState` gate before asserting outcome; (3) linger `2>/dev/null` swallowed failure — fixed with RC capture, fallback marker-file creation, and loud logging. Gold-standard test passed without reboot: `User session ready after 0s (runuser OK, D-Bus OK)` + `=== App update complete ===`. Boot tests: PASS 123 / FAIL 30 / WARN 2 / SKIP 0 (FAIL count unchanged, all 7 F-0123 assertions green). AC1/AC4 cold-boot validation still deferred to next reboot. |
