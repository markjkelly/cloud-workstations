...
| F-0089 | Add apt auto-upgrade for antigravity in boot scripts | [F-0040](specs/F-0040-antigravity-2-cli.md) | P1 | backlog | — | — | F-0087 | NOT YET IMPLEMENTED. Need new boot script `06b-apt-upgrade.sh` to run `apt-get install -y --only-upgrade antigravity` on every boot. Antigravity is single apt package with rolling 1.x releases (no "2.0" package exists). This item was discovered during live testing as necessary to keep workstation current. |
...
| F-0094 | CLI binary discovery via PATH | [F-0040](specs/F-0040-antigravity-2-cli.md) | P1 | done | SWE-3 | main | F-0088 | Verified `~/.local/bin` already in PATH in `workstation-image/boot/05-shell.sh` — curl installer places CLI artifacts in `~/.gemini/antigravity-cli/` (not a separate PATH-discoverable binary). No PATH change needed. |
