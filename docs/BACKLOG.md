...
| F-0123 | 07-apps user-session wait timeout (120s) too short — app updates skipped on slow boots | [F-0123](specs/F-0123-app-updates-systemd-ordering.md) | P1 | done | SWE-1 | feature/f0123-app-updates-systemd | F-0121 | **Done (2026-06-02).** Created `ws-app-updates.service` ordered `After=user@1000.service network-online.target` (created by `03-sway.sh` on every boot). Added `loginctl enable-linger user` to `03-sway.sh` so `user@1000.service` is reliable at boot. Skipped `07-apps.sh` from `setup.sh` inline loop. Kept `wait_for_user_session` as a defensive backstop. 6 F-0123 tests added to `10-tests.sh` (5 static + 1 best-effort runtime). `~/boot/` synced (diff clean). Timeout-raise stopgap explicitly rejected. AC1/AC4 reboot validation deferred to next deployment — cannot reboot live host. `docs/STARTUP_SCRIPTS.md` updated. |
